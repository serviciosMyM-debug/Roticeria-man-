    ...
    <article className="card overflow-hidden">
      ...
      <div className="space-y-4 p-5">
        <div className="flex flex-wrap gap-2">
          ...
          {product.isPromo ? <span className="badge bg-brand/15 text-brand-dark">Promo</span> : null}
          {product.isDailyMenu ? <span className="badge bg-black text-white">Menú del día</span> : null}
        </div>
        <div>
          <h3 className="text-xl font-black uppercase">{product.name}</h3>
          <p className="mt-1 text-sm text-zinc-600">{product.shortDescription}</p>
        </div>
        <div className="flex items-center justify-between gap-3">
          <div>
            <p className="text-2xl font-black text-brand-dark">{formatCurrency(finalPrice)}</p>
            {product.promoPrice ? (
              <p className="text-sm text-zinc-400 line-through">{formatCurrency(Number(product.price))}</p>
            ) : null}
          </div>
          <div className="flex gap-2">
            <Link href={`/product/${product.slug}`} className="btn-secondary px-4 py-2">
              Ver
            </Link>
            <button
